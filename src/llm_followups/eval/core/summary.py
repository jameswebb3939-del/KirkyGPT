...
def summarise_results(results: Sequence[EvaluatedExample]) -> dict[str, Any]:
    ...
    final_format = evaluator_summary.get("followup_format")
    if final_format is not None:
        summary["format_valid_percentage"] = final_format["pass_rate_percentage"] or 0.0

    raw_format = evaluator_summary.get("raw_followup_format")
    if raw_format is not None:
        summary["raw_format_valid_percentage"] = raw_format["pass_rate_percentage"] or 0.0
